...
/// <summary>
/// Configuration for ArdinCode.
/// </summary>
public class ArdinCodeConfig
{
    /// <summary>
    ...
    /// When enabled, ArdinCode detects light/dark theme and applies a curated color palette.
    /// </summary>
    ...
    /// Defaults to ~/.ardincode/skills. Each skill is a folder containing a SKILL.md file.
    /// </summary>
    ...
    /// Defaults to ./.ardincode/skills relative to the project root. Project skills
    ...
    /// When enabled, ArdinCode spawns/connects to each entry in <see cref="McpServers"/> at startup
    ...
    /// <summary>
    /// Resolves the effective user-skills directory, falling back to ~/.ardincode/skills.
    /// </summary>
    public string GetEffectiveUserSkillsDirectory()
    {
        ...
        return Path.Combine(userProfile, ".ardincode", "skills");
    }

    /// <summary>
    /// Resolves the effective project-skills directory. If the configured path is relative,
    /// it is resolved against <paramref name="projectRoot"/>. Defaults to ./.ardincode/skills.
    /// </summary>
    ...
    /// </summary>
    public static ArdinCodeConfig Load(string? configPath = null)
    {
        ...
        if (File.Exists(configPath))
        {
            try
            {
                ...
                var config = JsonSerializer.Deserialize<ArdinCodeConfig>(json, ConfigJsonOptions.ReadOptions);
                if (config != null)
                {
    ...
    {
        var userProfile = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
        return Path.Combine(userProfile, ".ardincode", "config.json");
    }
    ...
    /// </summary>
    public static ArdinCodeConfig CreateDefault()
    {
        return new ArdinCodeConfig
        {
            ApiEndpoint = "https://api.avalai.ir/v1",
...
/// remote HTTP servers populate <see cref="Url"/>. ArdinCode-only extensions
