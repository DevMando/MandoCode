...
class Program
{
    static async Task Main(string[] args)
    {
        ...
        // ardincode won't connect; readme points users at it.
        ...
        hostBuilder.ConfigureServices(services =>
        {
            ...
            {
                var cfg = provider.GetRequiredService<ArdinCodeConfig>();
            ...
            services.AddSingleton(provider => new McpClientManager(provider.GetRequiredService<ArdinCodeConfig>()));
            services.AddSingleton(provider => new McpApprovalGate(provider.GetRequiredService<ArdinCodeConfig>()));
            ...
            {
                var cfg = provider.GetRequiredService<ArdinCodeConfig>();
                var ignoreDirs = new HashSet<string>(ArdinCodeConfig.DefaultIgnoreDirectories);
            ...
            // Commands are sourced from ArdinCode.Models.SlashCommands so that the
    ...
    {
        var problems = 0;
        Console.WriteLine("ArdinCode Doctor");
        Console.WriteLine("================");
        ...
        Console.WriteLine(problems == 0
            ? "All checks passed."
            : $"{problems} problem(s) found. Run ardincode and use /setup to fix interactively.");
    ...
    static async Task HandleConfigCommandAsync(string[] args)
    {
        ...
        switch (command)
        {
            ...
            case "init":
            case "create":
                var newConfig = ArdinCodeConfig.CreateDefault();
                newConfig.Save();
                Console.WriteLine($"✓ Created default configuration at: {ArdinCodeConfig.GetDefaultConfigPath()}");
                newConfig.Display();
                break;
            ...
            case "set":
                if (args.Length < 3)
                {
                    Console.WriteLine("Usage: ardincode config set <key> <value>");
                    return;
                }
    ...
    {
        Console.WriteLine("ArdinCode Configuration\n");
    ...
    static async Task SetConfigValueAsync(string key, string value)
    {
        ...
        if (!result.Ok)
        {
            Console.WriteLine("Run 'ardincode --config --help' for available keys");
            return;
        }
        ...
        Console.WriteLine($"Configuration saved to: {ArdinCodeConfig.GetDefaultConfigPath()}");
    }
}
