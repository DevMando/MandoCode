...
public static class OllamaSetupHelper
{
    ...
    /// browser" step still sees `where ollama` fail in the running ardincode session.
    ...
    ///     (especially when ArdinCode's VDOM render loop is repainting the screen),
    ...
    public static async Task<CloudAuthState> CheckCloudSignInAsync(string url, CancellationToken ct = default)
    {
        ...
        return models.Any(ArdinCodeConfig.IsCloudModel)
