...
public sealed class OnboardingFlow
{
    ...
    public async Task<FlowResult> RunAsync(ArdinCodeConfig config, bool forceInteractive = false, CancellationToken ct = default)
    {
        ...
        var recommendedCtx = ArdinCodeConfig.RecommendedContextLength(pickedModel);
        if (recommendedCtx > 0 && recommendedCtx != config.ContextLength)
        {
            config.ContextLength = recommendedCtx;
            AnsiConsole.MarkupLine($"[dim]Context window sized to {recommendedCtx / 1024}k tokens for this model tier (adjust: ardincode --config set contextLength <n>).[/]");
        }
        ...
        if (ArdinCodeConfig.IsCloudModel(pickedModel))
        {
    ...
    private static void DisplayHeader()
    {
        var panel = new Panel(
            Align.Center(
                new Markup("[bold deepskyblue1]Welcome to ArdinCode[/]\n[dim]Let's get you set up — takes about a minute[/]"),
    ...
    private async Task<OllamaSetupHelper.ProbeResult> EnsureDaemonRunningAsync(ArdinCodeConfig config, CancellationToken ct)
    {
        ...
        // user has pointed ardincode at a remote Ollama box.
        while (true)
        {
    ...
    private async Task<string?> PickModelAsync(string url, List<string> models, CancellationToken ct)
    {
        ...
        var labeled = models
            .OrderBy(m => ArdinCodeConfig.IsCloudModel(m) ? 0 : 1)
            .ThenBy(m => m, StringComparer.OrdinalIgnoreCase)
            .Select(m => ArdinCodeConfig.IsCloudModel(m)
                ? $"{m}   (cloud)"
                : $"{m}   (local)")
            .ToArray();
    ...
    private async Task<string?> PickWhenEmptyAsync(string url, CancellationToken ct)
    {
        ...
        var source = Select("Which would you like?",
            $"Cloud (recommended — auto-installs {ArdinCodeConfig.DefaultCloudModel})",
        ...
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine($"[dim]Cloud tip: [white]{ArdinCodeConfig.DefaultCloudModel}[/] outperforms all of these and needs no GPU (free with [deepskyblue1]ollama signin[/]).[/]");
        AnsiConsole.WriteLine();
        ...
        AnsiConsole.MarkupLine($"[dim]Tip: [white]{ArdinCodeConfig.DefaultCloudModel}[/] is more powerful and needs no GPU (free with [deepskyblue1]ollama signin[/]).[/]");
    ...
    private async Task<string?> EnsureCloudModelAsync(string url, CancellationToken ct)
    {
        ...
        var pulled = await PullModelWithProgressAsync(ArdinCodeConfig.DefaultCloudModel, ct);
        if (!pulled)
        {
            AnsiConsole.MarkupLine($"[yellow]Pull didn't finish cleanly. If you're not signed in, run [deepskyblue1]ollama signin[/] then retry with: [deepskyblue1]ollama pull {ArdinCodeConfig.DefaultCloudModel}[/][/]");
            return null;
        }

        AnsiConsole.MarkupLine($"[green]✓ Pulled {ArdinCodeConfig.DefaultCloudModel}[/]");
        return ArdinCodeConfig.DefaultCloudModel;
    }
    ...
    public async Task<bool> WalkThroughCloudSigninAsync(string url, CancellationToken ct = default)
    {
        ...
        AnsiConsole.WriteLine();
        AnsiConsole.MarkupLine("ArdinCode can sign you in by running [deepskyblue1]ollama signin[/] for you — your browser will");
