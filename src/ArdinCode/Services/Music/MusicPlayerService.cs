...
{
    private readonly ArdinCodeConfig _config;
    ...
    public MusicPlayerService(ArdinCodeConfig config)
    {
        _config = config;
    ...
    /// <summary>
    /// Creates ~/.ardincode/music/lofi/ and ~/.ardincode/music/synthwave/ if they don't exist.
    /// </summary>
    ...
    /// <summary>
    /// Discovers MP3 tracks from embedded resources and the user's ~/.ardincode/music/ folder.
    /// Embedded resource names follow: {RootNamespace}.Audio.{genre}.{filename}.mp3
    /// User tracks follow: ~/.ardincode/music/{genre}/{filename}.mp3
    /// </summary>
    private void DiscoverTracks()
    {
        ...
        // 2. User's custom tracks from ~/.ardincode/music/{genre}/*.mp3
    ...
    public bool Play(string? genre = null)
    {
        ...
        if (genreTracks.Count == 0)
        {
            ...
            if (genreTracks.Count == 0)
            {
                AudioError = $"No MP3 files found. Drop .mp3 files into ~/.ardincode/music/{{genre}}/ (e.g. {_userMusicPath}/lofi/)";
                return false;
            }
        }
