...
public class WebSearchPlugin
{
    ...
    private const string TavilySignupHint =
        ...
        "then run: /config set tavilyKey <key>. The key is stored locally in ~/.ardincode/config.json " +
    ...
    /// ArdinCodeConfig.GetEffectiveTavilyApiKey before constructing). Null/blank = DuckDuckGo only.
    /// </param>
    ...
    private static string BuildDuckDuckGoBlockedMessage(string reason, bool tavilyConfigured)
    {
        ...
        return msg + $"\nTell the user: for reliable web search, ArdinCode supports Tavily — a search API " +
    ...
    public static async Task<string> ValidateTavilyKeyAsync(string apiKey)
    {
        try
        {
            using var request = BuildTavilyRequest(apiKey, "ArdinCode API key verification", maxResults: 1, includeAnswer: false);
            using var response = await _httpClient.SendAsync(request);
