...
- **Windows (cmd.exe):** warns against unix tools (`head`, `grep`, `cat`, `ls`), prefers ArdinCode's own file tools (`read_file_contents`, `search_text_in_files`, `list_files_match_glob_pattern`).
...
CLI equivalents: `ardincode --config set autoContinue false`, `--config set toolBudget 200000`, `--config set timeout 30`.
...
| `Models/SystemPrompts.cs` | `BuildArdinCodeAssistant(webSearchEnabled)` (the main prompt; shell rules are appended at runtime) |
...
Unit tests cover the deterministic parts: `TaskPlannerServiceTests`, `InvocationScopeTests`, `PlanHandoffTests`, `ShellEnvironmentTests`, `ArdinCodeConfigTests`. End-to-end flows are manual.
